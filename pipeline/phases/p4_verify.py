...
def run_p4(config: BuildConfig, claude: ClaudeClient,
    ...
    Reads atoms_deduplicated.json from P3, samples based on quality tier,
    looks up baseline evidence via Seekers, calls Claude for verification.
    """
    ...
    try:
        ...
        try:
            ...
            raise PhaseError(phase_id, f"P3 output not found: {input_path}")
        ...
        # Select atoms to verify
        if sample_pct >= 100:
            atoms_to_verify = list(all_atoms)
        else:
            sample_size = max(1, int(len(all_atoms) * sample_pct / 100))
            atoms_to_verify = random.sample(all_atoms, min(sample_size, len(all_atoms)))

        # Track which atom IDs were verified
        verified_ids = set()
        ...
        for i, atom in enumerate(atoms_to_verify):
            progress = int((i / max(total_to_verify, 1)) * 85)
            ...
            # If no evidence and no lookup — mark with lower confidence, skip Claude
            if not evidence_text:
                atom["status"] = "verified"
                atom["confidence"] = max(0.4, float(atom.get("confidence", 0.5)) * 0.8)
                atom["verification_note"] = "No baseline evidence available — confidence reduced"
            ...
            try:
                ...
                new_confidence = float(result.get("confidence", atom.get("confidence", 0.5)))
                ...
                atom["baseline_reference"] = result.get("evidence_source", "")
                verified_ids.add(atom_id)

            except Exception as e:
                logger.warn(f"Claude verify failed for {atom_id}: {e}", phase=phase_id)
                atom["status"] = "verified"
        ...
        for atom in all_atoms:
            if atom.get("id") not in verified_ids:
                atom["status"] = "verified"
                atom.setdefault("verification_note", "Not sampled — passed through")

        # Calculate score
        if all_atoms:
            avg_confidence = sum(float(a.get("confidence", 0.5)) for a in all_atoms) / len(all_atoms)
            flagged_penalty = (flagged_count / max(total_to_verify, 1)) * 20
            score = min(100.0, max(0.0, avg_confidence * 100 - flagged_penalty))
        else:
            score = 0.0
